...
public class NotchTools implements INotchSupport {
    ...
    public void fullScreenDontUseStatus(Activity activity) {
        fullScreenNonUseStatus(activity, null);
    }
     ...
     */
    @Override
    public void fullScreenNonUseStatus(final Activity activity, final OnNotchCallBack notchCallBack) {
        if (notchScreenSupport == null) {
            checkScreenSupportInit(activity.getWindow());
        }
        if (notchScreenSupport != null) {
            notchScreenSupport.fullScreenNonUseStatus(activity, notchCallBack);
        }
    }

    public void fullScreenDontUseStatusForPortrait(Activity activity) {
        fullScreenNonUseStatusForPortrait(activity, null);
    }
    ...
    public void fullScreenDontUseStatusForLandscape(final Activity activity) {
        fullScreenDonUseStatusForLandscape(activity, null);
    }

    @Override
    public void fullScreenDonUseStatusForLandscape(final Activity activity, final OnNotchCallBack notchCallBack) {
        ThreadUtils.post2UI(new Runnable() {
            ...
            public void run() {
                ...
                if (notchScreenSupport != null) {
                    notchScreenSupport.fullScreenDonUseStatusForLandscape(activity, notchCallBack);
                }
            }
        });
    }
